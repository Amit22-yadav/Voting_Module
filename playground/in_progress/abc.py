...
app =(
     bk.Application("Voting_Contract_Ctrl_Alt",state = Proposal_Record_State())
...
def Registred_user_per_proposal(proposal_id:pt.abi.String,user_id:pt.abi.String,asset_id:pt.abi.String,token_count:pt.abi.Uint64,*,output:User_per_proposal_record)-> pt.Expr:
    registred_user = User_per_proposal_record()

    return pt.Seq(
    registred_user.set(proposal_id,user_id,asset_id,token_count),
    pt.If(app.state.proposal_rec[proposal_id.get()].exists()== pt.Int(0))
    .Then(
...
def Register_user_asset(user_id:pt.abi.String,asset_id:pt.abi.String,token_count:pt.abi.Uint64,*,output:User_asset_store)-> pt.Expr:
    asset_register = User_asset_store()

    return pt.Seq(
        # asset_register.decode(app.state.asset_rec[user_id.get()].get()),
         pt.If(app.state.user_rec[user_id.get()].exists()== pt.Int(0))
    .Then(
...
@app.external
def Update_Users(proposal_id:pt.abi.String,user_id:pt.abi.String,asset_id:pt.abi.String,token_count:pt.abi.Uint64,*,output:User_per_proposal_record)->pt.Expr:
    ...
    return pt.Seq(
        ...
        update_user_store.set(proposal_id,user_id,asset_id,token_count),
...
@app.external
def Voting_Record(proposal_id:pt.abi.String,user_id:pt.abi.String,token_count:pt.abi.Uint64,user_response:pt.abi.Bool,*,output:Response_store)-> pt.Expr:
    proposal_get = Proposal_Record()
    ...
    return pt.Seq(
        ...
        response_get.set(user_id,token_count,user_response),
